...
{
  m_calcAngle = g_robotContainer.m_drivetrain.GetGyroYaw() + m_angle;
}
...
void CmdDriveTurnToOffsetAngle::Execute() 
{
  ...
  double delta_angle = m_calcAngle - g_robotContainer.m_drivetrain.GetGyroYaw();
  ...
  if( delta_angle < 0)
    g_robotContainer.m_drivetrain.Drive(0,0,  -turn_power, Drivetrain::ROBOTCENTRIC);
  else
    g_robotContainer.m_drivetrain.Drive(0,0,   turn_power, Drivetrain::ROBOTCENTRIC);
}
